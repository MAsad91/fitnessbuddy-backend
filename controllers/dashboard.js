...
exports.todayCalories = async (req, res) => { console.log(req.user);
    try {
        ...
        const totals = todaysFood.reduce((acc, entry) => {
            // Use new structure with totalMacros
            if (entry.totals && entry.totals.totalMacros) {
                acc.totalCalories += entry.totals.totalMacros.calories || 0;
                acc.totalProtein += entry.totals.totalMacros.protein || 0;
                acc.totalCarbs += entry.totals.totalMacros.carbs || 0;
                acc.totalFats += entry.totals.totalMacros.fats || 0;
            } else {
                // Fallback for old structure (if any entries still use old format)
    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dayTotals = dayFood.reduce((acc, entry) => {
                if (entry.totals && entry.totals.totalMacros) {
                    acc.calories += entry.totals.totalMacros.calories || 0;
                }
                return acc;
            }, { calories: 0 });
